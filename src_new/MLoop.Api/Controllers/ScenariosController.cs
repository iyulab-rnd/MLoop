...
public class ScenariosController : ControllerBase
{
    ...
    public async Task<IActionResult> Train(string scenarioId, [FromBody] TrainRequest request)
    {
        try
        {
            var scenario = await _scenarioService.GetScenarioAsync(scenarioId);
            ...
            var jobId = await _scenarioService.CreateTrainJobAsync(scenarioId, request.Command);
            return Ok(new { jobId });
        }
        catch (Exception ex)
        {
    ...
    public async Task<IActionResult> Predict(string scenarioId)
    {
        try
        {
            ...
            if (scenario.Models.Count == 0)
                return BadRequest("No trained models available");

            var jobId = await _scenarioService.CreatePredictJobAsync(scenarioId);
            return Ok(new { jobId });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error creating predict job for scenario {ScenarioId}", scenarioId);
            return BadRequest(ex.Message);
        }
    }
}